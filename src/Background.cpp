...
void Background::ObjectsLoad()
{
	...
	for (int i = 0; i < LastCloudIdx; ++i) {
		BgObj[i].Load(FP::BACKGROUND::BG, 2000.0f, 2000.0f, AEVec2{ 40, 30.0f });
		
	}
	BgObj[5].Load(FP::BACKGROUND::Moon, 0.0f, 0.0f, AEVec2{ 40, 30.0f });
	BgObj[6].Load(FP::BACKGROUND::Moon, 0.0f, 0.0f, AEVec2{ 760, 30.0f });
	BgObj[6].ReflectAboutYAxis();

	SceneColors[static_cast<int>(SceneType::Day)] = Color{ 117.0f, 137.0f, 136.0f, 255.0f };
	SceneColors[static_cast<int>(SceneType::SunSet)] = Color{ 117.0f, 137.0f, 136.0f, 255.0f };
	SceneColors[static_cast<int>(SceneType::Night)] = Color{ 117.0f, 137.0f, 136.0f, 255.0f };
}
