...
void Tiles::CheckOutOfBound(void) {
	...
	{
		SetColliders();
		if (image.pos.y >= static_cast <f32> (9999)) 
			active = false;
	}
}
void Tiles::SetColliders()
{
	...
	if (type == TileType::Grass)
		collider.top.pos = AEVec2Set(image.pos.x,image.pos.y - image.height / 2.0f + collider.top.height / 2.0f + (5.5f / TILE_CONST::GRASS_SPRITE_HEIGHT * image.height));
	else
